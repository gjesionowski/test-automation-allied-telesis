...
host1=$(dialog --title "$uut Test Configuration: Host A" --radiolist "Select (with Space) the OS family of Test PC 1:" 0 0 2>&1 1>&3 \
Windows "(All versions)" on \
Linux "(All versions)" off \
);
host2=$(dialog --title "$uut Test Configuration: Host B" --radiolist "Select (with Space) the OS family of Test PC 2:" 0 0 2>&1 1>&3 \
Windows "(All versions)" on \
Linux "(All versions)" off \
);
